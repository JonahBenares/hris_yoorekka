<?php
    ...
    $return = array('bday' => date("F d, Y",strtotime($row['bdate'])), 'civil_status' => $row['civil_status'], 'date_hired' => date("F d, Y",strtotime($row['date_hired'])), 'corporate_name' => $corporate_name, 'sss' => $sss, 'philhealth' => $philhealth, 'pagibig' => $pagibig,'tin' => $tin, 'designation' => $designation,'end_contract' => $end_contract, 'date_reg' => date("F d, Y",strtotime($date_reg)), 'department' => $department, 'salary' => $salary, 'allowance' => $allowance, 'emp_status' => $row['emp_status'], 'current_supid' => $row['current_supervisor'], 'current_supervisor' => $current_supervisor); 
